...
st.caption("Preencha os dados abaixo para receber sua projeção personalizada — leva menos de 2 minutos.")
st.markdown("---")
...
with st.form("simulation_form"):
    ...
    with c1:
        current_age = st.number_input(
            "Idade atual", min_value=18, max_value=79, value=35,
            help="Sua idade hoje"
        )
    with c2:
        retirement_age = st.number_input(
            "Idade para aposentar", min_value=40, max_value=99, value=65,
            help="Quando você quer parar de trabalhar"
        )
    with c3:
        life_expectancy = st.number_input(
            "Expectativa de vida", min_value=60, max_value=120, value=90,
            help="Use um valor conservador. IBGE aponta ~80 anos; use 90 para mais segurança."
        )
    ...
    with c4:
        current_savings = st.number_input(
            "Patrimônio atual (R$)", min_value=0.0, value=50_000.0,
            step=5_000.0, format="%.0f",
            help="Soma de todos os seus investimentos e poupança"
        )
    with c5:
        monthly_contribution = st.number_input(
            "Aporte mensal (R$)", min_value=0.0, value=1_000.0,
            step=100.0, format="%.0f",
            help="Quanto você pode poupar/investir por mês"
        )
    with c6:
        monthly_expenses = st.number_input(
            "Despesa mensal atual (R$)", min_value=500.0, value=5_000.0,
            step=500.0, format="%.0f",
            help="Seus gastos mensais hoje"
        )

    st.subheader("📐 Premissas Econômicas")
    c7, c8, c9 = st.columns(3)
    with c7:
        retirement_pct = st.slider(
            "Custo de vida na aposentadoria (%)", 50, 100, 80,
            help="Porcentagem dos gastos atuais que você espera ter aposentado. "
                 "Recomendação geral: 70–80%."
        )
    with c8:
        inflation_rate = st.number_input(
            "Inflação anual estimada (%)", 1.0, 20.0, 4.5, step=0.1,
            help="Meta do Banco Central: 3–4,5%. Use 5% para cenário conservador."
        )
    with c9:
        investment_return = st.number_input(
            "Retorno nominal anual (%)", 1.0, 30.0, 9.0, step=0.5,
            help="Retorno esperado da sua carteira. CDI ~10,5%, IPCA+5 ~9,5%, carteira mista ~10%."
        )

    st.subheader("🏛️ INSS / Previdência Pública")
    c10, c11 = st.columns(2)
    with c10:
        has_inss = st.checkbox("Contribuo ao INSS ou regime próprio de previdência", value=True)
    with c11:
        inss_benefit = st.number_input(
            "Benefício estimado do INSS (R$/mês, valor atual)",
            min_value=0.0, max_value=15_000.0, value=2_500.0, step=100.0,
            disabled=not has_inss,
            help="Consulte o extrato no Meu INSS (meu.inss.gov.br). Se não souber, use R$ 1.412 (média 2024)."
        )
    if not has_inss:
        inss_benefit = 0.0
...
if submitted:
    ...
    # Gap mensal em dinheiro de hoje
    monthly_gap_today = max(0.0, monthly_expenses * retirement_pct / 100 - inss_benefit)

    # Gap nominal na data da aposentadoria
    monthly_gap_at_ret = monthly_gap_today * (1 + inf_annual) ** years_to_ret

    # Capital-alvo na aposentadoria (VP de anuidade com retorno real durante a aposentadoria)
    # Assume que as despesas crescem com inflação e o capital rende a taxa nominal → usa retorno real
    if real_monthly > 1e-9:
        ...
        # Cenário degenerado: retorno = inflação → sem crescimento real
    ...
    # Valor futuro dos aportes mensais (anuidade ordinária)
    if r_monthly > 1e-12:
        fv_contribs = (
    ...
    # Aporte mensal necessário para atingir a meta
    ...
    m1.markdown(
        f'<div class="result-card" style="background:linear-gradient(135deg,#1e3c72,#2a5298)">'
        f'<div class="value">R$ {target_capital/1e6:.2f}M</div>'
        f'<div class="label">🎯 Capital-alvo na aposentadoria</div></div>',
        unsafe_allow_html=True,
    )
    m2.markdown(
        f'<div class="result-card" style="background:linear-gradient(135deg,#667eea,#764ba2)">'
        f'<div class="value">R$ {total_accumulated/1e6:.2f}M</div>'
        f'<div class="label">📈 Estimativa acumulada</div></div>',
        unsafe_allow_html=True,
    )
    ...
    if on_track:
        st.markdown(
            ...
            f'<h4>✅ Você está no caminho certo!</h4>'
            f'<p>Mantendo o aporte de <strong>R$ {monthly_contribution:,.0f}/mês</strong>, você acumulará '
            f'<strong>R$ {total_accumulated:,.0f}</strong> — suficiente para cobrir a meta de '
        ...
        st.markdown(
            ...
            f'<p>Para atingir a meta, você precisaria aumentar o aporte para '
            f'<strong>R$ {required_monthly:,.0f}/mês</strong> '
            f'(aumento de <strong>R$ {required_monthly - monthly_contribution:,.0f}/mês</strong>).</p>'
            f'<p style="margin-top:8px">Alternativas: aumentar gradualmente os aportes, revisar a carteira '
            f'para maior retorno, ou ajustar a meta de despesas na aposentadoria.</p></div>',
            unsafe_allow_html=True,
        )
    ...
    d3.metric("Despesa mensal (na apos.)", f"R$ {monthly_gap_at_ret + (inss_benefit*(1+inf_annual)**years_to_ret):,.0f}")
    d4.metric("Cobertura estimada do INSS",f"R$ {inss_benefit*(1+inf_annual)**years_to_ret:,.0f}")
    ...
    fig.add_trace(go.Scatter(
        x=ages, y=acc,
        mode="lines", name="Patrimônio com rendimentos",
    ...
    fig.add_trace(go.Scatter(
        x=ages, y=pure,
        mode="lines", name="Apenas aportes (sem rendimentos)",
    ...
    if on_track:
        recs.append(("✅ Manter a disciplina",
            "Você está no caminho certo! Revise esta simulação anualmente e após grandes mudanças de vida "
            "(casamento, filhos, promoção, mudança de emprego)."))
    else:
        recs.append(("🔴 Aumentar aportes mensais",
            f"Você precisaria contribuir com **R$ {required_monthly:,.0f}/mês** para atingir a meta. "
            "Mesmo um aumento gradual de R$ 100–200/mês já faz diferença significativa no longo prazo."))
    if investment_return < 7.0:
        recs.append(("📈 Revisar a carteira de investimentos",
            "Com retorno abaixo de 7% a.a., sua carteira pode estar muito conservadora. "
            "Tesouro IPCA+, fundos multimercado e ETFs podem elevar o retorno sem risco excessivo."))
    if inss_benefit < monthly_expenses * retirement_pct / 100 * 0.5:
        recs.append(("🏛️ Previdência privada complementar",
            "O INSS cobre menos de 50% das suas despesas projetadas na aposentadoria. "
            "Um plano PGBL (se declaração completa) ou VGBL complementa de forma eficiente."))
    if current_savings < monthly_expenses * 6:
        recs.append(("💰 Reserva de emergência primeiro",
            "Antes de aumentar aportes em previdência, garanta uma reserva de 3–6 meses de despesas "
            "em investimentos líquidos (ex.: Tesouro Selic, CDB com liquidez diária)."))
    if years_to_ret > 25:
        recs.append(("⏳ Aproveite o tempo a seu favor",
            f"Com {years_to_ret} anos pela frente, cada real investido hoje tem altíssimo potencial de crescimento. "
            "Pequenos ajustes agora evitam grandes esforços no futuro."))
    ...
    if not on_track:
        lines += [
            ...
            f"  Aumento necessário:        R$ {required_monthly-monthly_contribution:,.2f}/mês",
        ]
    lines += [
        "",
        "=" * 58,
        "  ⚠️  Este relatório é informativo e não constitui",
        "      recomendação de investimento. Consulte um CFP®.",
        "=" * 58,
    ]
    ...
    st.info(
        "📞 Quer análise personalizada? Fale com nossos especialistas via "
        "[WhatsApp](https://wa.me/5511999999999) ou volte à página inicial para
